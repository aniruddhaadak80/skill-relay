...
import type { Compatibility, EngineResult, PackStatus, RelayPack } from "@/lib/types";
...
export function PackDetail({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (scoreResponse.ok) setEngine(await scoreResponse.json() as EngineResult);
    } catch (loadError) {
  ...
  const slug = pack.selectedSkillName.toLowerCase().replace(/[^a-z0-9-]+/g, "-");

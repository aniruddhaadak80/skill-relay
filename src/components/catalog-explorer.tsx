...
export function CatalogExplorer({ initial }: { initial: CatalogResponse }) {
  ...
  return <div className="explorer-shell">
    <div className="explorer-toolbar paper-card">
      <div className="search-field"><Search size={18} /><input value={query} onChange={(event) => handleQueryChange(event.target.value)} placeholder="Search 179k+ public skills, tools, or workflows" aria-label="Search public skills" /><kbd>⌘ K</kbd>{loading ? <Loader2 size={16} className="spin" /> : null}</div>
      <button type="button" className="btn-secondary filter-toggle" onClick={() => setMinimumFit(minimumFit ? 0 : 70)}><SlidersHorizontal size={16} /> {minimumFit ? `fit ≥ ${minimumFit}` : "Filters"}</button>
      <button type="button" className="btn-primary" onClick={() => void refresh()}><Sparkles size={16} /> Refresh signals</button>
    </div>
    <div className="filter-drawer paper-card">
      <div className="filter-title"><Filter size={16} /><span>Advanced filters</span><small>{filtered.length} visible / {items.length} loaded</small></div>
      ...
      <label>Source<select value={source} onChange={(event) => setSource(event.target.value)}><option value="all">All sources</option><option value="claudskills">ClaudSkills mirror</option><option value="skills.sh">skills.sh</option><option value="sealed-fallback">Sealed fallback</option></select></label>
      <label>Harness<select value={harness} onChange={(event) => setHarness(event.target.value)}><option value="all">Any harness</option>{HARNESSES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label>Minimum fit <strong>{minimumFit || "any"}</strong><input type="range" min="0" max="95" step="5" value={minimumFit} onChange={(event) => setMinimumFit(Number(event.target.value))} /></label>
      <label>Sort<select value={sort} onChange={(event) => setSort(event.target.value)}><option value="fit">Highest fit</option><option value="installs">Most installs</option><option value="fresh">Freshest source</option></select></label>
    </div>
    {error ? <div className="error-strip"><X size={15} />{error}</div> : null}
    <div className="explorer-grid">
      <div className="catalog-column">
        <div className="results-head"><div><span className="eyebrow">public index</span><h2>{query ? `Results for “${query}”` : "Fresh signal packets"}</h2></div><SourceBadge live={initial.live} label={initial.sourceLabel} /></div>
        <div className="catalog-list">
          {filtered.map((skill, index) => <button type="button" className={selected?.id === skill.id ? "skill-card selected" : "skill-card"} key={skill.id} onClick={() => setSelected(skill)} style={{ "--card-index": index } as React.CSSProperties}>
